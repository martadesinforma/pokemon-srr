...
export class PokemonsService {
  ...
  //Este método se encarga de cargar un pokemon en concreto manejando los errores de la solicitud HTTP mediante el método privado handleError. Se va a llamar en el componente pokemon-page
  public loadPokemon(id: string): Observable<Pokemon> {
    ...
    return this.http.get<Pokemon>( //Este return es el principal y devuelve el Observable generado por la llamada HTTP.
      ...
      .pipe(
        catchError(this.handleError) //para interceptar y manejar cualquier error mediante el método handleError.
      )
  };

  //Este método maneja los errores de la solicitud HTTP. Distingue entre errores del cliente (cuando error.status === 0) y errores del servidor.
  private handleError(error: HttpErrorResponse) {
    ...
    //Convierte el error en un mensaje y lanza una nueva excepción Error usando throwError(() => new Error(errorMessage)), permitiendo que el error sea manejado posteriormente en la suscripción de quien llama
